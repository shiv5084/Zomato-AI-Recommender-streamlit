...
- Define Stable JSON request/response for “recommend”:
...
- Implement HTTP endpoints(v1 intent):
...
### Suggested Implementation
- e.g. frontend/ + README section “Run the web app”.
- Key components:
  - `PreferenceForm.jsx`
...
### Goal
Provide an alternative, lightweight deployment path using Streamlit for rapid prototyping, demos, and simplified hosting without the full React + FastAPI stack.

### Core Responsibilities
- Build a single-page Streamlit app (`streamlit_app.py` or `app.py`) that directly orchestrates the recommendation pipeline.
- Implement UI widgets mapped to Phase 2 preference fields:
  - `st.selectbox` for location
  - `st.selectbox` or `st.radio` for budget band
  - `st.multiselect` for cuisines
  - `st.slider` for minimum rating
  - `st.text_area` for additional preferences
- Integrate directly with Phase 1 (data ingestion),validate (Phase 2), Phase 3 (filtering), Phase 4 (LLM recommendation) and Phase 5 (semantics) within the Python process—no separate backend required.
- Display results using Streamlit native components:
  - `st.metric` or `st.write` for filter/candidate counts
  - `st.expander` or custom `st.container` blocks for ranked restaurant cards with name, cuisine, rating, cost, and AI explanation
...
- Complementary: Phase 7 remains the primary product UI (browser + REST). 
- Handle empty states and errors gracefully with `st.info`, `st.warning`, or `st.error`.
- Add a `requirements-streamlit.txt` or equivalent dependency set and document launch command (`streamlit run app.py`).
- Optionally support Streamlit Cloud deployment (`requirements.txt` + `.streamlit/config.toml`).

### Exit Criteria
- A user can install dependencies and launch the app with a single command.
- The Streamlit app produces the same logical recommendations as the CLI/API for identical inputs.
- UI is responsive enough for demos ,ad-hoc usage,mobile and web.
- Deployment instructions are added to README.
- README (or a short docs/streamlit-deploy.md) documents how to run locally (streamlit run …) and how to deploy to Community Cloud (repo layout, secrets names, branch); a reviewer can open the hosted URL and complete one successful recommendation or see an intentional empty state.

### Suggested Implementation
- Root-level or `src/phase8/streamlit/` folder containing `app.py`, repo root streamlit_app.py (Cloud entrypoint).
- Reuse existing internal packages (phase1, phase2, phase3, phase4) directly; do not duplicate business logic.
- Cache dataset loading with `st.cache_data` or `st.cache_resource` to avoid repeated Hugging Face downloads.
- Use `st.spinner` during LLM calls for visual feedback.

## Phase 9: Deployment, Hardening and Handoff
...
- Phase 9 (Streamlit) depends on Phases 0-5 and can be developed in parallel with Phases 6-7 as an alternative deployment path.
